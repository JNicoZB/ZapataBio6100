...
# create an empty vector, specify mode & lenght
...
rep(x=0.5, times=6) # using the argument names os always prudent
...
`:`(2,4)
seq(from=2,to=4,by=0.5) # usea a by function can generate real numbers
seq(from=2,to=4,by=0.67)
x <- seq(from=2, to=4,length=7) # sometimes easir to just specify the length
print(x)
...
rnorm(6) # 6 randonm normal values with mean 0 and standard deviation 1 
...
sample(x=long_vec, size=10) # specify a number (sampling without replacement)
...
print(my_weights)
sample(x=long_vec,replace=TRUE,prob=my_weights) # sampling with replacemnt and wights
...
# this works, but is overkill; just usea the boolea
z[which(z<3)]

# can also usea length() for relative posotioning to last element
...
zD[zd < 0.5] # NA values carriend along!
